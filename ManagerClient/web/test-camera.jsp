        ...
        <div class="row bg-white radius-top-10">
            <div class="col-12 pr-5 pl-4 ">
                <div id="table-component" class="table-row h-100-per">
                    <x:forEach var="camera" select="$doc/cameras/camera">
                        <div id="<x:out select="$camera/@id"/>" class="col-3 h-50 mb-3 pointer item" style="border: 1px #a2a2a2 solid;">
                            <div class="item-body h-80-per text-center">
                                <img src="<x:out select="$camera/@image"/>" title="<x:out select="$camera/@name"/>" style="width: 200px; height: 200px"/>
                                <div class="name px-5 h-15-per">
                                    <span><x:out select="$camera/@name"/></span>
                                </div>
                                <div class="price">
                                    <span>€ <x:out select="$camera/@price"/></span>
                                </div>
                                <div class="megapixel">
                                    <span>MP: <x:out select="$camera/@megapixel"/></span>
                                </div>
                            </div>
                            <div class="item-footer h-20-per text-center">
                                <img width="50" src="WEB-RENDER/src/assets/menu/camera.png"/>
                            </div>
                        </div>
                    </x:forEach>
                </div>
